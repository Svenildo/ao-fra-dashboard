// collectors/run-extended.js
import ExtendedCollector from './data-collector-extended.js';
...
console.log('⏳ Collecte Extended démarrée...');
...
async function once() {
  ...
  isRunning = true;
  try {
    const collector = new ExtendedCollector(walletPath);
    inFlight = collector.collectAndSend();
    await inFlight;
    console.log('✅ Collecte Extended terminée');
  } catch (err) {
    console.error('❌ Erreur durant la collecte Extended:', err?.message || err);
  } finally {
...
async function shutdown(sig) {
  ...
  console.log('👋 Extended runner stoppé proprement.');
  process.exit(0);
}
